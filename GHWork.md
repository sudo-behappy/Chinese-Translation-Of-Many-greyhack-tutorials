...
- 如果你不知道密码，你必须入侵目标。点击右边的 [如何远程入侵](#如何远程入侵)并阅读它。
- 你需要提供shell权限的漏洞来连接任何东西。
- 如果漏洞描述说获得访问shell的权限，你可以用它来连接。否则，你不能连接...
- 如果你已经连接了并且不知道该怎么做，点击右边的 [如何本地入侵](#如何本地入侵)并阅读它。
...
- 侵入并进入任何邮件服务器。
- 你可以在数据库中找到账户和密码。
- 如果你得到了数据库，你可以打开并查看它或运行破译和破解密码。
---

## 如何入侵路由器
- 运行scanrouter来了解目标路由器的版本号。
- 从黑客商店下载一个kernel_router漏洞。
- 运行漏洞。
- 你可以用路由器漏洞入侵服务器/计算机。
...
- 单击 "添加条目Add Entry"按钮，并像这样填写空白的文件。
- 单击 "防火墙Firewall"。
- 单击 "添加条目 "按钮。
...
- 点击[如何生存](#如何生存)，并阅读它。
- 点击[如何防范嗅探器](#如何防范嗅探器)并阅读它。
- 点击[如何防范反向shell](#如何防范反向shell)并阅读它。
---
...
- 不要忘记清除可疑的日志。阅读游戏手册中的 "游戏结束 "或点击右边的 "如何清除日志 "并阅读它。
- 点击[如何自保](#如何自保)并阅读它。
...
## 如何升级本地库
  运行这两个命令（点击右边的apt-get，并阅读它以了解更多细节）。
...
## 如何搭建特定服务器
- 访问普通商店或黑客商店以找到所需文件。
- 如果你不知道在哪里找到所需的文件，请点击右边的 "设置服务器所需的文件和端口 "并阅读它。
- 找到所需的文件，然后下载它。
...
| SERVICE NAME | REQUIRED FILE | WHERE TO FIND | PORT NUMBER |
|---|---|---|---|
